...
class ModelServingClient(object):
    ...
    def __init__(self, host="localhost", port_number=9999, callback=None, stats=False, stats_name="model_client", compression_level=1, cache=True):
        ...
        self.throttle_limit = 3  # In windows.
    ...
    def encode_numpy2img(self, window_meta, inputs):
        ...
        for i in range(len(byte_list), len(inputs)):
            # jpeg is a lossy but fast compression than png i.e. png maintains the quality
            img = inputs[i]
            buf = io.BytesIO()
            # img.save(buf, "JPEG") # JPEG is a lossy compression and has substantial impact on the accuracy.
    ...
    def predict(self, window_meta, input_data, needs_serialization=True):
        """
