...
# MobileNet v1.0
elif [ "${model}" == "mobilenet-early-discard" ]; then
echo "Starting mobile-early-discard model..."
python3 main.py --model mobilenet --modality RGB --resume_path ${dataset_root}/model_ckpt/mobilenet-early-discard_window_${sample_duration}_size_${sample_size}.pth --sample_duration ${sample_duration} --sample_size ${sample_size} --num_classes 1 --model_depth 1 --port_number ${port_number} --workers ${num_workers} # --enable_stats
fi
