...
python3 main.py --model resnet --resnet_shortcut A --modality RGB --resume_path ${dataset_root}/model_ckpt/resnet-18_window_${sample_duration}_size_${sample_size}.pth --sample_duration ${sample_duration} --sample_size ${sample_size} --num_classes 51 --model_depth 18 --port_number ${port_number} # --enable_stats

# Resnext-101
elif [ "${model}" == "resnext-101" ]; then
echo "Starting resnext-101 model..."
python3 main.py --model resnext --resnet_shortcut B --resnext_cardinality 32 --modality RGB --resume_path ${dataset_root}/model_ckpt/resnext-101_window_${sample_duration}_size_${sample_size}.pth --sample_duration ${sample_duration} --sample_size ${sample_size} --num_classes 51 --model_depth 101 --port_number ${port_number} # --enable_stats
fi
