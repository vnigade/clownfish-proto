...
class ModelServingServicer(predict_pb2_grpc.ModelServingServicer):
    ...
    def pre_process(self, input, input_size, window_meta):
        # print("Preprocessing:", input_size, window_meta[ID])
        window_size = window_meta[END_FRAME] - window_meta[START_FRAME] + 1
        if self.transforms is not None:
            ...
            if overlap_frames:
                ...
                if new_start_frame < window_size:
                    ...
                    if input_size[0] == window_size:
                        input = self.unserialize(
                            input, input_size, start_index=new_start_frame)
                    else:
                        input = self.unserialize(
                            input, input_size, start_index=0)
                    unserialize_time = (
    ...
    def predict(self, request, context):
        ...
        # print("Received window id", window_meta_pb.id, input_size)
        # Preprocess input
        ...
        # print("Sending window id", window_meta_pb.id, (timeit.default_timer() - received_start_time) * 1000)
